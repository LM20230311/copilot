...
- **事件类型**：`add-start`、`add-progress`、`add-end`、`edit-start`、`edit-progress`、`edit-end`、`delete-start`、`delete-progress`、`delete-end`、`cmd`、`chat`
- **数据格式**：JSON
- **编码方式**：UTF-8
- **操作类型**：`add`、`edit`、`delete`、`cmd`、`chat`、`show-start`、`show-end`
...
- 命令执行事件：`cmd`（无阶段区分）
- 聊天事件：`chat`（无阶段区分，通过 `data.type` 进一步区分展示行为）
...
interface BaseOperationData {
  type: 'add' | 'edit' | 'delete' | 'cmd' | 'chat' | 'show-start' | 'show-end';  // 操作类型
...
```typescript
interface ChatOperationData extends BaseOperationData {
  type: 'chat' | 'show-start' | 'show-end';
  content?: string;           // AI 聊天内容或状态说明
  filePath?: string;          // show-start/show-end 时标识关联文件
  meta?: {
    messageRole?: 'assistant' | 'user';
    displayMode?: 'text' | 'card';
    cardId?: string;          // loading 卡片唯一标识
  };
}
```
...
1. **聊天消息 (`type: 'chat'`)**
   - 表示纯文本聊天回复，客户端应直接展示 `content`。
   - 当 `meta.displayMode === 'card'` 时可选择放入卡片展示但无需等待文件处理状态。

2. **文件处理卡片开始 (`type: 'show-start'`)**
   - 携带 `filePath`（必需）以及可选 `meta.cardId`。
   - 客户端应展示对应文件的“处理中”卡片，并与 `operationId` 关联以便更新状态。

3. **文件处理卡片结束 (`type: 'show-end'`)**
   - 同样包含 `filePath` 与 `meta.cardId`。
   - 客户端根据 `meta.status`（可选：`'success' | 'error'`）决定卡片展示结果，随后可关闭 loading 状态。

> 通过 `data.type` 判断是否为聊天类事件：`chat` 代表直接展示内容，`show-start`/`show-end` 则标记与某个文件相关的加载卡片开始与完成。
...
```json
{"event":"chat","messageId":"msg_chat_001","operationId":"op_chat_001","data":{"type":"chat","content":"已经开始分析 project/src/app.ts，请稍等。","meta":{"messageRole":"assistant","displayMode":"text"}}}
{"event":"chat","messageId":"msg_chat_001","operationId":"op_file_123","data":{"type":"show-start","filePath":"src/app.ts","content":"正在解析文件内容……","meta":{"cardId":"card_app_ts","displayMode":"card"}}}
{"event":"chat","messageId":"msg_chat_001","operationId":"op_file_123","data":{"type":"show-end","filePath":"src/app.ts","content":"文件处理完成 ✔","meta":{"cardId":"card_app_ts","status":"success","displayMode":"card"}}}
```
...
```json
{
  ...
  "operationId": "op_chat_001",
  "data": {
    "type": "chat",
    "content": "已经开始分析 project/src/app.ts，请稍等。",
    "meta": {
...
#### show-start 事件（展示文件处理卡片）

```json
{
  "event": "chat",
  "messageId": "msg_chat_001",
  "operationId": "op_file_123",
  "data": {
    "type": "show-start",
    "filePath": "src/app.ts",
    "content": "正在解析文件内容……",
